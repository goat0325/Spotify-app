...
@section('content')
    ...
    <!-- 流れの確認のため、「ゲスト」でホーム画面に入れる設定。認証されていないユーザーでもホーム画面にアクセスできるように -->
        @if ($user)
            ...
            <!-- 認証ユーザー専用のボタンや機能 -->
            <img src="{{ $user->profile_image ?? asset('images/default.png')  }}" alt="{{ $user->Spotify_user_id }}" width="50" height="50">
            <span>{{ $user->Spotify_user_id }}</span>
            <p>Welcome, {{ $user->Spotify_user_id }}!</p>

            <!-- ログアウトボタン -->
            <form action="{{ secure_url('logout') }}" method="POST" style="display:inline;">
                @csrf
